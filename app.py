...
top_options =  [{'label': 'Top 20', 'value': 'Top 20'},
                        ...
                        {'label': 'Top 40', 'value': 'Top 40'}] # dropdown option for top countries
    ...
    children=[
            ...
            children = [
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        value ='Confirmed',
                        style={'width':'49%', 'margin-left': '195px',
                ...
                html.Div(
                    ...
                    # Giving column heading for the right panel table
                    #Column-Name-Country/Province-Confirmed-Death-Recovered
                    html.Div(
                    children = [
                        #Country/Province
                        html.Div([html.P('Country/Province',
                        style={'text-align':'center','font-family':'sans-serif',
                        'width':'18vh', 'font-size':'12px', 'color':'#303030'})]),
                        #Confirmed
                        html.Div(
            ...
            html.Div(
                children = [
                    ...
                    html.Div([
                        ...
                        #Drop-Down-2-Div
                        html.Div([
                            html.P('Select the date to view'
                            ,style={'text-align':'center','font-family':'sans-serif',
                            'color': colors['text']}),
                            dcc.Dropdown(
                            id='optionbubble',
                            ...
                            value ='Confirmed',
                            style={'width':'49%', 'margin-left': '200px',
                            'text-align':'left', 'color':colors["text1"]}
                            ),
                            html.Div(style = {"padding":3}),# For vertical space
                    ...
                    style = {"height":"110vh",'padding-left':'20px','padding-right':'5px'},
                    className = "panel columns div-right-panel")
            ],style={"display": "flex", "flex-direction": "row"}),
            #<----------------------End-Part-2 Tab 1--------------------------------->
        ],
        ...
        children = [
            ...
            html.Div([ # Title for the second part in tab 2
                ...
                html.P("Hover over the provinces to view the trends in province-wise confirmed and death cases",
                    style = small_italics)
            ]),
            ...
            html.Div([ # Div tag for Canada Bubble Map
                html.Div([
                    dcc.Graph(id='canada1',config={'displayModeBar':False},
                    style = {"height":"50vh"},
                    hoverData={'points': [{'customdata': 'Alberta'}]}),
                ],style = graph_style,className="six columns"),
                html.Div([
                    dcc.Graph(id='canada2',config={'displayModeBar':False},
                    style = {"height":"50vh"},
                    hoverData={'points': [{'customdata': 'Alberta'}]}),
                ],style = graph_style,className="six columns"),
            ],className="row"),
            ...
            html.Div([ # Div tag for line and bar chart
                html.Div([
                        html.Div([
                            dcc.Graph(id='canada1-line',config={'displayModeBar':False},
                            style = {"height":"40vh"}),
                        ],className="three columns"),
                        html.Div([
                            dcc.Graph(id='canada1-bar',config={'displayModeBar':False},
                            style = {"height":"40vh"}),
                        ],className="three columns"),
                ]),
                html.Div([# Div tag for line and bar chart
                        html.Div([
                            dcc.Graph(id='canada2-line',config={'displayModeBar':False},
                            style = {"height":"40vh"}),
                        ],className="three columns"),
                        html.Div([
                            dcc.Graph(id='canada2-bar',config={'displayModeBar':False},
                            style = {"height":"40vh"}),
                        ],className="three columns"),
                    ]),
                ],style = graph_style),
...
def update_div(clickData):
    """
    Function to update the total number of confirmed cases on top-right panel,
    based one click data from Choropleth
    Arguments:
        clickData: Click data of country selected from Choropleth
    Returns:
        Updated column total cases for top right panel
    """
    country = clickData['points'][0]['customdata']
    if country == "Syria" or country == "France":
        country = ""
...
def update_div(clickData):
    """
    Function to update the total number of death cases on the top-right Panel
    based on the click data from Choropleth
    Arguments:
        clickData: Click data of country selected from Choropleth
    Returns:
        Updated column total death cases for top right panel
    """
    country = clickData['points'][0]['customdata']
    if country == "Syria" or country == "France":
        country = ""
...
def update_div(clickData):
    """
    Function to update the total number of recovered cases on the top-right Panel
    based on the click data from Choropleth
    Arguments:
        clickData: Click data of country selected from Choropleth
    Returns:
        Updated column total recovered cases for top right panel
    """
    country = clickData['points'][0]['customdata']
    if country == "Syria" or country == "France":
        country = ""
        ...
        for i in range(10):
            ...
            return_divs.append(
                html.Div(children = [html.P(top["Country/Region"] ,
                        ...
                        style={'text-align':'center','font-family':'sans-serif',
                        'color': colors[case], 'font-size': '24px'})],
                        className = "topcount"))
        ...
        for i in range(10):
            ...
            return_divs.append(
                html.Div(
                          ...
                          'font-family':'sans-serif', 'color': colors[case], 'font-size': '24px'})],
                          className = "topcount"))
...
def update_figure(hoverData):
    ...
    fig = go.Figure(go.Scatter(x=province_df['Date'], y=province_df['Confirmed'],
                    ...
                    name=province))
    fig.update_layout(
    title= province + " Total Confirmed Cases: " + '{:,d}'.format(province_df['Confirmed'].max()) ,
    title_x= 0.5,
    titlefont= dict(
        color=colors['text1'],
        size=18
...
def update_figure(hoverData):
    ...
    fig.update_layout(
    title= province + " New Confirmed Cases: "+ '{:,d}'.format(province_df.loc[province_df['Date'] == latest_date,'New Confirmed'].iloc[0]),
    title_x= 0.5,
    titlefont= dict(
        color=colors['text1'],
        size=18
  ...
  for country,noofdays, confirmed, deaths, recovered, new_cases
    ...
    for ydn, yd, xd in zip(current_df[y_axis].iloc[limit:], count_df['NoOfDays'].iloc[limit:], count_df['Country/Region'].iloc[limit:]):
        ...
        annotations.append(dict(xref='x1', yref='y1', y=xd, x=yd+3,
                                ...
                                font=dict(family='Arial', size=12, color=colors['graph_text']),
                                showarrow=False))
        annotations.append(dict(xref='paper', yref='paper', x=0.1, y=0.98,
